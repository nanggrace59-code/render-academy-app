...
import { StudentWithSubmission, ClassType, Profile, Submission } from '@/types';
import { User, AlertCircle, ChevronRight, Search, Users } from 'lucide-react';
...
export default function TeacherDashboard() {
  ...
  useEffect(() => {
    Promise.all([getAllStudents(), getAllSubmissions()]).then(([profiles, submissions]) => {
      const data: StudentWithSubmission[] = profiles.map(student => {
        ...
        // Sort safely
        const latestSub = pendingSub || studentSubs.sort((a,b) => (b.assignment_number || 0) - (a.assignment_number || 0))[0];
  ...
  // Note: Ensure your students in Supabase actually have 'enrolled_class' set, 
  // otherwise this might return empty. For demo, if column is missing, you might want to show all.
  const filteredStudents = students.filter(s => s.enrolled_class === selectedClass || !s.enrolled_class);
        ...
        {filteredStudents.length === 0 ? (
          ...
          <div className="divide-y divide-neutral-800">
            {filteredStudents.map((student) => (
              <div 
                key={student.id} 
                onClick={() => student.latest_submission && router.push(`/teacher/grading/${student.latest_submission.id}`)}
