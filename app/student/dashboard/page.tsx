...
export default function StudentDashboardWrapper() {
    ...
    return (
        // FIXED: flex-col to stack Header on top of Content (No Sidebars)
        <div className="min-h-screen bg-[#050505] text-neutral-200 font-sans selection:bg-[#d90238] selection:text-white flex flex-col overflow-hidden">
...
function StudentWorkspace({ user, setUser }: { user: Profile, setUser: (u: Profile) => void }) {
    ...
    if (!user.references?.interior || !user.references?.exterior) {
        return (
            <div className="flex-1 flex flex-col items-center justify-center p-6 bg-[#050505] pb-32">
                <div className="max-w-4xl w-full text-center space-y-10">
                     <div>
                        <span className="text-[#d90238] text-[10px] font-bold tracking-[0.3em] uppercase border border-[#d90238]/30 px-4 py-1.5 rounded-full bg-[#d90238]/5">Project Initialization</span>
                        <h1 className="text-5xl font-black text-white mt-6 mb-2">Upload Master References</h1>
                     ...
                     <div className="grid grid-cols-2 gap-8">
                        {['interior', 'exterior'].map((type) => (
                            <div key={type} className="group relative aspect-video bg-[#0a0a0a] border border-white/5 rounded-2xl overflow-hidden hover:border-[#d90238]/50 transition-all cursor-pointer">
                     ...
                     <div className="flex justify-center pt-4">
                        <button onClick={saveRefs} disabled={isInitSaving || (!refPreviews.interior || !refPreviews.exterior)} className="bg-[#d90238] hover:bg-[#b0022d] text-white px-12 py-4 rounded-xl font-bold uppercase tracking-widest text-xs w-full max-w-md disabled:opacity-30 disabled:cursor-not-allowed">
    ...
    return (
        <div className="flex-1 flex flex-col h-screen overflow-hidden relative">
            ...
            <header className="h-20 border-b border-white/5 bg-[#050505] flex items-center justify-between px-8 shrink-0 z-50">
                 ...
                 {/* LEFT: Navigation Buttons (Replaces "R" Logo and old Nav) */}
                 <div className="flex items-center gap-4">
                    <button 
                        onClick={() => setViewMode('workspace')} 
                        className={`px-6 py-2.5 rounded-full flex items-center gap-2 text-xs font-bold uppercase tracking-widest transition-all ${viewMode === 'workspace' ? 'bg-[#d90238] text-white shadow-[0_0_20px_rgba(217,2,56,0.3)]' : 'bg-transparent text-neutral-500 hover:text-white'}`}
                    >
                        <MonitorPlay size={16}/> Workspace
                    </button>
                    <button 
                        onClick={() => setViewMode('gallery')} 
                        className={`px-6 py-2.5 rounded-full flex items-center gap-2 text-xs font-bold uppercase tracking-widest transition-all ${viewMode === 'gallery' ? 'bg-[#d90238] text-white shadow-[0_0_20px_rgba(217,2,56,0.3)]' : 'bg-transparent text-neutral-500 hover:text-white'}`}
                    >
                        <LayoutGrid size={16}/> Gallery
                    </button>
                 </div>

                 {/* RIGHT: System Icons (Replaces User Profile) */}
                 <div className="flex items-center gap-6">
                    <button 
                        onClick={handleResetReferences} 
                        disabled={isResetting}
                        className="text-neutral-500 hover:text-[#d90238] transition-colors disabled:opacity-50" 
                        title="Settings / Reset"
                    >
                        {isResetting ? <Loader2 className="animate-spin" size={20}/> : <Settings size={20}/>}
                    </button>
                    <button onClick={() => { localStorage.removeItem('activeUserEmail'); window.location.href = '/login'; }} className="text-neutral-500 hover:text-white transition-colors" title="Logout">
                        <LogOut size={20}/>
                    </button>
                 </div>
            </header>

            {/* --- CONTENT (NO LEFT SIDEBAR HERE) --- */}
