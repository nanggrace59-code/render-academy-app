            ...
            {!needsInitialization && (
                <div className="w-20 bg-[#0a0a0a] border-r border-white/5 flex flex-col items-center py-0 shrink-0 z-50 animate-in fade-in slide-in-from-left-4 duration-500">
                    <div className="h-20 flex items-center justify-center w-full mb-6">
                        <div className="w-10 h-10 bg-[#d90238] rounded-lg flex items-center justify-center font-black text-white text-[10px] tracking-tighter leading-none shadow-[0_0_15px_rgba(217,2,56,0.3)]">RTA</div>
                    </div>
                    <div className="flex flex-col gap-6 items-center w-full">
                        <button onClick={() => setViewMode('workspace')} className={`w-10 h-10 rounded-lg flex items-center justify-center transition-all ${viewMode === 'workspace' ? 'bg-white text-black shadow-lg scale-105' : 'text-neutral-500 hover:bg-white/5 hover:text-white'}`} title="Workspace"><MonitorPlay size={20}/></button>
                        <button onClick={() => setViewMode('gallery')} className={`w-10 h-10 rounded-lg flex items-center justify-center transition-all ${viewMode === 'gallery' ? 'bg-white text-black shadow-lg scale-105' : 'text-neutral-500 hover:bg-white/5 hover:text-white'}`} title="Gallery"><LayoutGrid size={20}/></button>
                    </div>
                ...
                <header className={`h-20 border-b border-white/5 bg-[#050505] flex items-center justify-between px-8 shrink-0 z-40 ${needsInitialization ? 'px-12' : ''} animate-in fade-in slide-in-from-top-4 duration-500`}>
                     <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            {subTitle && (<p className="text-[10px] text-[#d90238] font-bold uppercase tracking-[0.2em] mt-1">{subTitle}</p>)}
                        </div>
                     </div>
                    ...
                    {needsInitialization ? (
                        <div className="w-full h-full flex flex-col items-center justify-center p-6 bg-[#050505] animate-in fade-in zoom-in-95 duration-500">
                            <div className="max-w-4xl w-full text-center space-y-10">
                                <div><span className="text-[#d90238] text-[10px] font-bold tracking-[0.3em] uppercase border border-[#d90238]/30 px-4 py-1.5 rounded-full bg-[#d90238]/5 shadow-[0_0_15px_rgba(217,2,56,0.1)]">Project Initialization</span><h1 className="text-5xl font-black text-white mt-6 mb-2 tracking-tight">Upload Master References</h1><p className="text-neutral-500 text-sm">These will serve as the ground truth for all future assignments.</p></div>
                                <div className="grid grid-cols-2 gap-8">
                                    {['interior', 'exterior'].map((type) => (
                                        <div key={type} className="group relative aspect-video bg-[#0a0a0a] border border-white/5 rounded-2xl overflow-hidden hover:border-[#d90238]/50 transition-all cursor-pointer shadow-2xl">
                                            {!(type === 'interior' ? refPreviews.interior : refPreviews.exterior) && (<input type="file" accept="image/*" onChange={(e) => handleRefChange(e, type as any)} className="absolute inset-0 z-20 opacity-0 cursor-pointer" />)}
                                            {(type === 'interior' ? refPreviews.interior : refPreviews.exterior) ? (<><img src={type === 'interior' ? refPreviews.interior : refPreviews.exterior} className="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity"/><button onClick={() => removeRef(type as any)} className="absolute top-3 right-3 z-30 w-8 h-8 bg-black/50 backdrop-blur hover:bg-[#d90238] rounded-full flex items-center justify-center text-white transition-colors"><X size={14}/></button></>) : (<div className="absolute inset-0 flex flex-col items-center justify-center group-hover:scale-105 transition-transform duration-500"><span className="text-xs font-bold uppercase tracking-[0.2em] text-neutral-500 group-hover:text-white transition-colors">CLICK TO UPLOAD {type}</span></div>)}
                                        </div>
                                    ))}
                                </div>
                                <div className="flex justify-center pt-4"><button onClick={saveRefs} disabled={isInitSaving || (!refPreviews.interior || !refPreviews.exterior)} className="bg-[#d90238] hover:bg-[#b0022d] text-white px-12 py-4 rounded-xl font-bold uppercase tracking-widest text-xs w-full max-w-md disabled:opacity-30 disabled:cursor-not-allowed shadow-[0_0_20px_rgba(217,2,56,0.2)] hover:shadow-[0_0_30px_rgba(217,2,56,0.4)]">{isInitSaving ? 'INITIALIZING...' : 'INITIALIZE PROJECT'}</button></div>
                            </div>
                        </div>
                    ) : (
                        // --- MAIN VIEW SWITCHER ---
