...
function StudentWorkspace({ user }: { user: Profile }) {
    ...
    return (
        <div className="flex-1 flex overflow-hidden relative">
            ...
            {/* LEFT THIN SIDEBAR NAVIGATION */}
            <div className="w-16 bg-[#0a0a0a] border-r border-white/5 flex flex-col items-center justify-between py-6 shrink-0 z-50">
                 <div className="flex flex-col gap-4">
                    <button onClick={() => setViewMode('workspace')} className={`w-10 h-10 rounded-lg flex items-center justify-center transition-all ${viewMode === 'workspace' ? 'bg-[#d90238] text-white shadow-glow' : 'text-neutral-500 hover:bg-white/5 hover:text-white'}`} title="My Workspace"><MonitorPlay size={20}/></button>
                    <button onClick={() => setViewMode('gallery')} className={`w-10 h-10 rounded-lg flex items-center justify-center transition-all ${viewMode === 'gallery' ? 'bg-[#d90238] text-white shadow-glow' : 'text-neutral-500 hover:bg-white/5 hover:text-white'}`} title="Student Gallery"><LayoutGrid size={20}/></button>
                 </div>
                 
                 {/* "GO BACK" / RESET REFERENCES BUTTON */}
                 <div className="flex flex-col gap-4">
                    <button 
                        ...
                        title="Change Master References (Go Back)"
                    >
