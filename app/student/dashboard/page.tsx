...
export default function StudentDashboardWrapper() {
    ...
    return (
        <div className="min-h-screen bg-[#050505] text-neutral-200 font-sans selection:bg-[#d90238] selection:text-white overflow-hidden flex">
            {/* The layout is now handled inside Workspace to allow sidebar/header interaction */}
...
function StudentWorkspace({ user, setUser }: { user: Profile, setUser: (u: Profile) => void }) {
    ...
    // --- 1. INITIALIZATION SCREEN (Full Screen Overlay) ---
    if (!user.references?.interior || !user.references?.exterior) {
        return (
            <div className="w-full h-screen flex flex-col items-center justify-center p-6 animate-in fade-in duration-500 bg-[#050505]">
    ...
    return (
        <div className="flex w-full h-screen overflow-hidden bg-[#050505]">
            ...
            <div className="w-20 bg-[#0a0a0a] border-r border-white/5 flex flex-col items-center justify-between py-6 shrink-0 z-50">
                 
                 {/* TOP: Gallery Button (Replacing Grid Icon) */}
                 <div className="flex flex-col gap-6 items-center">
                    <button 
                        onClick={() => setViewMode('gallery')} 
                        className={`w-10 h-10 rounded-lg flex items-center justify-center transition-all ${viewMode === 'gallery' ? 'bg-white text-black' : 'text-neutral-500 hover:bg-white/5 hover:text-white'}`} 
                        title="Gallery"
                    >
                        <LayoutGrid size={20}/>
                    </button>
                 ...
                 {/* BOTTOM: Settings & Logout (Replacing User Profile) */}
            ...
            <div className="flex-1 flex flex-col relative min-w-0">
                ...
                <header className="h-20 border-b border-white/5 bg-[#050505] flex items-center justify-between px-8 shrink-0 z-40">
                     {/* LEFT: WORKSPACE BUTTON (Replacing RTA Red Box) */}
                     <button 
                        onClick={() => setViewMode('workspace')}
                        className={`flex items-center gap-3 px-6 py-2.5 rounded-full transition-all ${viewMode === 'workspace' ? 'bg-[#d90238] text-white shadow-[0_0_20px_rgba(217,2,56,0.3)]' : 'bg-white/5 text-neutral-500 hover:text-white'}`}
                     >
                        <MonitorPlay size={16}/>
                            ...
                            <div className="flex-1 flex flex-col bg-[#020202] relative min-w-0 items-center justify-center p-8">
                                ...
                                <div className="w-full max-w-[1600px] aspect-video bg-black relative border border-white/5 rounded-2xl overflow-hidden shadow-2xl">
                                    {currentRefImage && currentRenderImage ? (
                                        ...
                                        <div className="w-full h-full flex flex-col items-center justify-center opacity-20"><p className="text-
